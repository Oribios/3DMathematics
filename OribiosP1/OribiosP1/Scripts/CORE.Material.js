...
function GL_CompileMaterial(vs_src, fs_src) {
    ...
    var m_Success = gl.getProgramParameter(material, gl.LINK_STATUS)
    if (m_Success == 0) {
        return null;
    } else {
        return material;
...
function Material() {
 ...
 Material.prototype.Build = function (mesh) {
        if (mesh.built == false) {
            ...
            mesh.normalBuffer.numItems = mesh.normals.length / 3;
    ...
    Material.prototype.Render = function (camera, mesh) {
        ...
        gl.uniform1i(this.shader.samplerColorUniform, 0);
